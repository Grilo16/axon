...
const Container = styled.div`
  ...
  padding-top: 16px; padding-bottom: 16px; padding-inline: 4px; width: 100%; height: 100%;
`;
...
const WorkspaceIcon = styled.div<{ $active?: boolean; $isDanger?: boolean }>`
  ...
  color: ${({ theme, $active, $isDanger }) => 
    $active ? "#fff" : 
    $isDanger ? "#f87171" : theme.colors.text.secondary};
  ...
  &:hover {
    ...
    background-color: ${({ theme, $active, $isDanger }) => 
      $active ? theme.colors.palette.primary : 
      $isDanger ? "rgba(248, 113, 113, 0.15)" : theme.colors.palette.secondary};
    ...
    <>
      <Container>
        {workspaces.map((ws: WorkspaceRecord) => {
          return (
            <WorkspaceLauncher key={ws.id} workspace={ws} /> 
          );
        })}
