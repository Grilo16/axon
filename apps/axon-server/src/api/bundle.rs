...
use axon_core::{
    bundler::rules::BundleOptions, domain::bundle::{
        BundleRecord, CloneBundleReq, CreateBundleReq, ListBundlesQuery, UpdateBundlePayload
    }, graph::AxonGraph
};


async fn verify_bundle_access(state: &AppState, bundle_id: &str, user_id: &str) -> AxonResult<BundleRecord> {
    let bundle = state.bundle_repo.get_by_id(bundle_id).await?
        .ok_or_else(|| AxonError::NotFound { entity: "Bundle", id: bundle_id.to_string() })?;
        
    let _ = state.workspace_repo.get_by_id_and_owner(&bundle.workspace_id, user_id).await?
...
pub async fn create_bundle(
    ...
    let _ = ctx.state.workspace_repo.get_by_id_and_owner(&payload.workspace_id, &ctx.user_id).await?
        .ok_or_else(|| AxonError::NotFound { entity: "Workspace", id: payload.workspace_id.clone() })?;
...
pub async fn get_workspace_bundles(
    ...
    let _ = ctx.state.workspace_repo.get_by_id_and_owner(&workspace_id, &ctx.user_id).await?
        .ok_or_else(|| AxonError::NotFound { entity: "Workspace", id: workspace_id.clone() })?;

    let limit = query.limit.unwrap_or(50); //
    let offset = query.offset.unwrap_or(0); //

    let records = ctx.state.bundle_repo.get_by_workspace_id(&workspace_id, limit, offset).await?; //
    Ok(Json(records))
}
...
pub async fn delete_bundle(
    ...
    let existing_bundles = ctx.state.bundle_repo.get_by_workspace_id(&bundle.workspace_id, 2, 0).await?; //
...
pub async fn clone_bundle(
    ...
    let cloned_record = ctx.state.bundle_repo.duplicate(&id, &new_id, payload.new_name).await?; //
