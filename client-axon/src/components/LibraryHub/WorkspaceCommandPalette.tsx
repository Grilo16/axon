...
const score = (q: string, ws: WorkspaceData) => {
  ...
  // simple scoring that feels great in practice:
  // exact prefix > substring; name > root
  ...
  // soft match by letters-in-order
  let i = 0;
...
export const WorkspaceCommandPalette = ({
  ...
  useEffect(() => {
    ...
    setQ("");
    setCursor(0);
    // focus next tick so modal portal exists
    setTimeout(() => inputRef.current?.focus(), 0);
  }, [isOpen]);

  useEffect(() => {
    // clamp cursor on changes
                ...
                >
                  ...
                  <div>
                    <Name>
                      {ws.name}{" "}
                      {isActive ? (
                        <Subtext style={{ marginLeft: 8 }}>(active)</Subtext>
                      ) : null}
                    </Name>
